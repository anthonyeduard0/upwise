...
function App() {
  ...
  const renderPage = useMemo(() => {
    ...
    switch (currentPage) {
      ...
      case 'rewards': return <RewardsPage user={user} setCurrentPage={setCurrentPage} />;
      case 'subscription': return <SubscriptionPage user={user} setUser={setUser} />; // Nova Rota
  ...
  const appContent = useMemo(() => {
    ...
    return (
      <div className="flex h-screen bg-gray-900 text-white font-inter">
        {user && <Sidebar currentPage={currentPage} setCurrentPage={setCurrentPage} user={user} isMobileMenuOpen={isMobileMenuOpen} toggleMobileMenu={toggleMobileMenu} onLogout={handleLogout} />}
