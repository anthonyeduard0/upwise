...
const Sidebar: React.FC<SidebarProps> = ({ currentPage, setCurrentPage, userName, userLevel, isMobileMenuOpen, toggleMobileMenu }) => {
  const navItems = [
  ...
  return (
    <>
      ...
      {/* Sidebar - Desktop e Mobile (condicional) */}
      <div 
        className={`fixed top-0 left-0 h-full w-64 bg-gray-900 text-white p-6 transform ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 transition-transform duration-300 ease-in-out z-40 md:static md:flex md:flex-col md:h-auto md:w-64 flex-shrink-0`}
      >
